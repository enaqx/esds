...
describe('Number', function() {
  describe('new esds.Number()', function() {
    it('should create new integer number', function() {
      var n1 = new esds.Number(1);
      var n2 = new esds.Number(1024);
      var n3 = new esds.Number(-1);
      var n4 = new esds.Number(-1024);
      var n5 = new esds.Number(0);
      var n6 = new esds.Number(-0);
      assert.ok(n1);
      assert.ok(n2);
      assert.ok(n3);
      assert.ok(n4);
    ...
    it('should create new floating-point number', function() {
      var n1 = new esds.Number(1.0);
      var n2 = new esds.Number(1.024);
      var n3 = new esds.Number(-1.0);
      var n4 = new esds.Number(-1.024);
      var n5 = new esds.Number(0.1);
      var n6 = new esds.Number(-0.2);
      assert.ok(n1);
      assert.ok(n2);
      assert.ok(n3);
      assert.ok(n4);
    ...
    it('should create new number on right string argument', function() {
      var n1 = new esds.Number('123');
      var n2 = new esds.Number('');
      var n3 = new esds.Number('0x11');
      assert.equal(n1, 123);
      assert.equal(n2, 0);
      assert.equal(n3, 17);
    });
  ...
  describe('Number properties', function() {
    it(`should have machine EPSILON equal to ` + 
        `${esds.Number.EPSILON}`, function() {
      assert.ok(esds.Number.EPSILON);
    });
    it(`should have the maximum safe integer MAX_SAFE_INTEGER equals to ` +
        `${esds.Number.MAX_SAFE_INTEGER}`, function() {
      assert.ok(esds.Number.MAX_SAFE_INTEGER);
    });
    it(`should have the largest positive number MAX_VALUE equals to ` +
        `${esds.Number.MAX_VALUE}`, function() {
      assert.ok(esds.Number.MAX_VALUE);
    });
    it(`should have the minimum safe integer MIN_SAFE_INTEGER  equals to ` +
        `${esds.Number.MIN_SAFE_INTEGER}`, function() {
      assert.ok(esds.Number.MIN_SAFE_INTEGER);
    });
    it(`should have the smallest positive number MIN_VALUE equals to ` +
        `${esds.Number.MIN_VALUE}`, function() {
      assert.ok(esds.Number.MIN_VALUE);
    });
    it('should have positive infinity POSITIVE_INFINITY', function() {
      assert.equal(esds.Number.POSITIVE_INFINITY, Infinity);
    });
    it('should have negative infinity NEGATIVE_INFINITY', function() {
      assert.equal(esds.Number.NEGATIVE_INFINITY, -1 * Infinity);
    });
  });

  describe('#isNaN()', function() {
    it('should determine whether the passed value is NaN', function() {
      assert.equal(esds.Number.isNaN(NaN), true);
      assert.equal(esds.Number.isNaN(1), false);
    });
  });
  ...
  describe('#isFinite()', function() {
    it('should determine whether the passed value is a finite number',
        function() {
      assert.equal(esds.Number.isFinite(Infinity), false);
      assert.equal(esds.Number.isFinite(2e64), true);
    });
  });

  describe('#isInteger()', function() {
    it('should determine whether the passed value is an integer', function() {
      assert.equal(esds.Number.isInteger(0.1), false);
      assert.equal(esds.Number.isInteger(1), true);
    });
  });
  ...
  describe('#isSafeInteger()', function() {
    it('should determine whether the provided value is a number ' +
      ...
      assert.equal(esds.Number.isSafeInteger(1), true);
      assert.equal(esds.Number.isSafeInteger(Math.pow(2, 53)), false);
    });
  });
  ...
  describe('#parseFloat()', function() {
    it('should parse a string argument and returns a floating point number',
        function() {
      assert.equal(esds.Number.parseFloat("3.14"), 3.14);
      assert.equal(esds.Number.parseFloat("314e-2"), 3.14);
      assert.equal(esds.Number.parseFloat("0.0314E+2"), 3.14);
      assert.equal(esds.Number.parseFloat("3.14more non-digit characters"),
        3.14);
    });
  });
  ...
  describe('#parseInt()', function() {
    it('should parses a string argument and returns an integer of the ' +
      ...
      assert.equal(esds.Number.parseInt("0xF", 16), 15);
      assert.equal(esds.Number.parseInt(15.99, 10), 15);
    });
  });

  describe('#toExponential()', function() {
    it('should return number in exponential notation', function() {
      var n = new esds.Number(12.345);
